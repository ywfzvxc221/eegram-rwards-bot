...
def send_welcome(message):
    bot.send_message(
        message.chat.id,
        "مرحباً بك! لإرسال طلب رشق، استخدم الأمر /طلب.\nلعرض الطلبات (للأدمن فقط): /الطلبات"
    )
...
def view_orders(message):
    ...
    for idx, order in enumerate(orders, start=1):
        text += (
            f"{idx}. المستخدم: {order['user_id']}\n"
            f"القسم: {order['section']}\n"
            f"الرابط: {order['link']}\n"
            f"الكمية: {order['quantity']}\n"
            f"التاريخ: {order['timestamp']}\n\n"
        )
