...
if not API_TOKEN or not ADMIN_ID:
    raise Exception("يرجى تحديد متغيرات BOT_TOKEN و ADMIN_ID في إعدادات البيئة (Environment Variables)")

# تحويل معرف الأدمن إلى رقم صحيح
...
def choose_withdraw_method(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    methods = [
        "الكريمي 🇾🇪", "فودافون كاش 🇪🇬", "شركة النجم للصرافة 🇾🇪", "شركة القطيبي للصرافة 🇾🇪",
        "الاهلي 🇸🇦", "الراجحي 🇸🇦", "بيبال 🇲🇦", "بنك CIH 🇲🇦",
        "بنك الاردن 🇯🇴", "زين كاش 🇮🇶", "محفظة Ooredoo 🇶🇦",
        "محفظة Jawwal 🇵🇸", "بنك لبنان والمهجر 🇱🇧"
    ]
    for i in range(0, len(methods), 2):
        markup.row(*[types.KeyboardButton(method) for method in methods[i:i+2]])
...
def handle_withdraw_choice(call):
    ...
    if call.data == "confirm_withdraw":
        bot.send_message(user_id, "✅ تم تأكيد السحب. سيتم مراجعته من قبل الأدمن.")
        data = user_withdraw_data[user_id]
        withdraw_entry = {
            "user_id": user_id,
            "amount": data["amount"],
        ...
        if os.path.exists("withdraw_requests.json"):
            with open("withdraw_requests.json", "r") as f:
                existing = json.load(f)
        else:
            existing = []
        existing.append(withdraw_entry)
    ...
    if os.path.exists("withdraw_requests.json"):
        ...
        for entry in data:
            if entry["user_id"] == user_id:
                entry["status"] = "مقبول" if action == "approve" else "مرفوض"
                break
        with open("withdraw_requests.json", "w") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    if action == "approve":
        bot.send_message(user_id, "✅ تم قبول طلب السحب الخاص بك. سيتم التنفيذ قريبًا.")
        bot.send_message(call.message.chat.id, f"تم قبول طلب السحب للمستخدم {user_id}.")
    else:
        bot.send_message(user_id, "❌ تم رفض طلب السحب الخاص بك.")
        bot.send_message(call.message.chat.id, f"تم رفض طلب السحب للمستخدم {user_id}.")
